...
# 1. 仅保留 Trace Agent 导入，注释掉 Log 和 Metric Agent
...
class DataDownloader:
    ...
    def __init__(self, force_refresh: bool = False, debug: bool = False):
        ...
        try:
            ...
            self.logger.info("✅ Trace Agent初始化完成 (Log/Metric 已禁用)")
    ...
    async def _download_single_problem_data(self, problem_data: Dict[str, Any]) -> bool:
        ...
        if not self.force_refresh and all(existing_data.values()) and existing_data:
    ...
    # 5. 保留 Log 和 Metric 的函数定义但暂时闲置（或者您可以直接删除）
    # 为了保持代码结构完整性，我保留了函数头，但它们不会被调用

    # async def _download_log_data(...)
    
    # async def _download_metric_data(...)
    ...
    async def _download_trace_data(self, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
        ...
        for evidence in evidence_chain.evidence:
            # 这里增加了对 'trace' 类型的宽容度检查，以防 agent 返回的是 'online_trace_analysis' 等
            if 'trace' in evidence.evidence_type and isinstance(evidence.data, list):
    ...
    async def download_all_problems(self, problems_file: str) -> None:
        ...
        self.logger.info("🚀 开始批量下载A榜观测数据 (Trace Only)")
    ...
    async def list_available_problems(self, problems_file: str) -> None:
        ...
        try:
            ...
            for i, problem in enumerate(problems, 1):
                ...
                status = "✅ Trace已下载" if existing_data.get('failure_traces.json') else "❌ 未下载"
