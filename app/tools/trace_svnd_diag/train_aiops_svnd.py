# train_aiops_svnd.py  (v0.3-nodectx, host GCN/GAT switchable)
# -*- coding: utf-8 -*-
"""
...
def main():
    ...
    ap.add_argument(
        "--save_pt",
        default="dataset/aiops_svnd_1209/1209/aiops_nodectx_multihead.pt",
        help="ckpt 路径",
    )
    ap.add_argument(
        "--host_conv",
        choices=["gcn", "gat"],
        default="gat",
        help="主机共址通道类型：gcn 或 gat",
    )
    ap.add_argument(
        "--type_min_support",
        type=int,
        default=200,
        help="细类参与训练/评测的最小支持数",
    )
    ...
    # infer host_state dimension from first record (if present)
    host_state_dim = 0
    if ds_fit.items:
        first = ds_fit.items[0]
        nodes0 = first.get("nodes") or []
        if nodes0:
            hs0 = nodes0[0].get("host_state")
            if isinstance(hs0, list) and len(hs0) > 0:
                host_state_dim = len(hs0)
    if host_state_dim > 0:
        print(f"[HostState] detected host_state_dim={host_state_dim}")
    keep_types = derive_keep_types(ds_fit.items, args.type_min_support)
    print(
        f"[TypeFilter] keep_types={sorted(list(keep_types))}  "
        f"(min_support={args.type_min_support})"
    )
    ...
    def run_epoch(loader, train: bool = True, show_progress: bool = True):
        model.train(train)
        tot = 0.0
        iterator = (
            tqdm(loader, total=len(loader), dynamic_ncols=True, leave=False)
            if show_progress
            else loader
        )
    ...
    for ep in range(1, args.epochs + 1):
        ...
        # 验证集总体指标（type 评测只在 m_type==1 样本上；并屏蔽未保留类）
        ...
        if vaL < best - args.early_stop_min_delta:
            ...
            if args.early_stop_patience > 0 and no_improve >= args.early_stop_patience:
                print(
                    f"[EarlyStop] 连续 {no_improve} 轮无显著改善，"
                    f"提前停止在 Epoch {ep:02d}"
                )
                break
